...
func main() {
	...
	fmt.Println(rec.Parsed.NameServers)       // [] — DENIC answers port 43 with just Domain+Status
	fmt.Println(rec.Parsed.Expiry)            // *time.Time (nil if the registry doesn't publish it)
}
```
